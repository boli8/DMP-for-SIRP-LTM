...
The protection resources are subject to the constraint: sum_[i] γ[t, i] <= γtot_each_t,
that is, there is a limited number of resources at each time t.
"""
...
o1, o2, γ_soln_ctr_a, opt_log = 
    gradient_descent_over_γ_mirror_SIRP_LTM_online_resource(objf_func, T, edge_list_a, adj_n_a, deg_a, σ0, βv, γ, μ, 
        adj_n_b, deg_b, b, θ, adj_n_Uab, adj_n_Λab, adj_n_Ea, adj_n_Eb, deg_Uab, deg_Λab, deg_Ea, deg_Eb, γtot_each_t, "gamma")

## Optionally, round up γ[t, i] to be integer:
require_integer_γ = false
if require_integer_γ
    γ_soln_ctr_a = round_up_γ_online_resource(T, no_of_nodes_a, γ_soln_ctr_a, γtot_each_t)
end
...
save_gamma = true
if save_gamma
    open(dir_result * "online_gamma_soln_ctr_a_at_gammatot_eacht$(γtot_each_t)" * 
         "_GA_" * graph_name_a * "_GB_" * graph_name_b * ".csv", "w") do io
        write(io, "i,sigma0,")
        for t in 1:T
            write(io, "t$(t),")
        end
...
require_integer_γ = false
if require_integer_γ
    γ_soln_ctr_ab = round_up_γ_online_resource(T, no_of_nodes_a, γ_soln_ctr_ab, γtot_each_t)
end
...
save_gamma = true
if save_gamma
    open(dir_result * "online_gamma_soln_ctr_ab_at_gammatot_eacht$(γtot_each_t)" * 
         "_GA_" * graph_name_a * "_GB_" * graph_name_b * ".csv", "w") do io
        write(io, "i,sigma0,")
        for t in 1:T
            write(io, "t$(t),")
...
savefig(dir_result * "exploring_online_mitigation.png")
